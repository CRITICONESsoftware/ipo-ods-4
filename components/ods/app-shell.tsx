...
export function AppShell() {
  const { currentPage, accessibility, setCurrentPage, setAccessibility, t } = useApp()
  ...
  useEffect(() => {
    ...
    // Advanced Voice Intelligence Engine (Isolated Scope)
      ...
      if (isActivating || isDeactivating) {
        for (const [key, prop] of Object.entries(accessToggles)) {
          if (finalTranscript.includes(key)) {
            ...
            // Priority to deactivate if both match, but normally only one will
  ...
  }, [accessibility.voiceNav, accessibility.language, setCurrentPage])
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const commands: Record<string, string> = { h: 'home', p: 'profile', v: 'video', a: 'accessibility', f: 'forum', d: 'donations', q: 'quiz', l: 'login', s: 'signup' }
      if (commands[key]) {
        e.preventDefault()
        setCurrentPage(commands[key] as any)
      }
    }
    window.addEventListener('keydown', handleKeyDown)
    return () => window.removeEventListener('keydown', handleKeyDown)
  }, [accessibility.keyboardOps, setCurrentPage])
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 relative overflow-hidden w-full ${accessibility.reducedMotion ? "[&_*]:!transition-none [&_*]:!animation-none" : ""}`} onMouseMove={(e) => {
          if (accessibility.readingGuide || accessibility.magnifier) {
            document.documentElement.style.setProperty('--mouse-y', `${e.clientY}px`)
            document.documentElement.style.setProperty('--mouse-x', `${e.clientX}px`)
          }
        }} >
        <div className="h-full overflow-y-auto transition-all duration-[400ms] ease-out pb-20"
