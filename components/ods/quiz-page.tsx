            ...
            {question.options.map((option, i) => {
              ...
              return (
                <a
                  ...
                  className={`w-full p-5 rounded-2xl text-left font-bold transition-all flex items-center justify-between group select-none block ${
                    variant === "default" && !showFeedback ? "bg-muted hover:bg-primary hover:text-primary-foreground" :
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {QUIZZES.map((quiz, index) => {
          ...
          return (
            <div 
              key={quiz.id}
              className={`flex flex-col bg-card border-2 rounded-[2.5rem] p-8 shadow-xl transition-all relative overflow-hidden group ${
                isUnlocked ? "border-border hover:border-primary cursor-pointer" : "border-border/30 opacity-60"
              }`}
              onClick={() => isUnlocked && onSelect(quiz.id)}
            >
              {/* Background accent */}
              <div className={`absolute top-0 right-0 w-24 h-24 rounded-full -mr-12 -mt-12 transition-transform duration-700 group-hover:scale-150 ${
                isCompleted ? "bg-green-500/10" : isUnlocked ? "bg-primary/5" : "bg-muted"
              }`} />
